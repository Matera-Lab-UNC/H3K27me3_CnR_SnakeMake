---
title: "Plots: DESeq2 pellet allFragmentBam PHO Analysis"
...
H3.2_pellet_allFrag_PHO_data <- read.csv("DESeq2_Results/20221205_K36R_HWT_pellet_IgGcon_DESeq2_allFragBam_PHO_results.csv", header=T, stringsAsFactors = F) %>%
                      dplyr::mutate(Genotype = 'H3.2', Fraction = 'Pellet')

head(H3.2_pellet_allFrag_PHO_data)

```

```{r read in H3.3 DESeq2 Pellet Fraction Data}

H3.3_pellet_allFrag_PHO_data <- read.csv("DESeq2_Results/20221205_Adelta_H33BK36R_pellet_IgGcon_DESeq2_allFragBam_PHO_results.csv", header=T, stringsAsFactors = F) %>%
                      dplyr::mutate(Genotype = 'H3.3', Fraction = 'Pellet')

head(H3.3_pellet_allFrag_PHO_data)

```

```{r Merge Pellet Data}

merged_Data <- rbind(H3.2_pellet_allFrag_PHO_data, H3.3_pellet_allFrag_PHO_data)
...
merged_Data %>%
  ...
  labs(title = 'H3.2 vs H3.3 Significant PHO Sites: All Fragment Pellet Fraction', y = 'PHO Sites', x  = 'Genotype', fill='Enrichment') +
...
merged_Data %>%
  ...
  facet_grid(~Chr) +
  labs(title = 'H3.2 vs H3.3 Significant PHO Sites per Chromosome: All Fragment Pellet Fraction', y = 'PHO Sites',x  = 'Genotype', 
       fill = 'Regulation') +
...
merged_Data %>%
  ...
  geom_point(data = function(x){x %>% dplyr::filter(padj < 0.05) %>% dplyr::filter(log2FoldChange < -1)},
             color = "#ff0000", 
             size=1, alpha=0.7) +
  labs(title = 'H3.2 vs H3.3 PHO Sites: All Fragment Pellet Fraction') +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
