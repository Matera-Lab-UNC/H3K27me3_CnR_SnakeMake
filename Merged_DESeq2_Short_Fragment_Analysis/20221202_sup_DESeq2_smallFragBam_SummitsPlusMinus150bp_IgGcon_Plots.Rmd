---
title: "Plots: DESeq2 sup smallFragmentBam Summits+-150bp Analysis"
...
H3.2_sup_smallFrag_summit_data <- read.csv("DESeq2_Results/20221205_K36R_HWT_sup_IgGcon_DESeq2_smallFragBam_SummitsPlusMinus150bp_results.csv", header=T, stringsAsFactors = F) %>%
                      dplyr::mutate(Genotype = 'H3.2', Fraction = 'Sup')

head(H3.2_sup_smallFrag_summit_data)

```

```{r read in H3.3 DESeq2 Sup Fraction Data}

H3.3_sup_smallFrag_summit_data <- read.csv("DESeq2_Results/20221205_Adelta_H33BK36R_sup_IgGcon_DESeq2_smallFragBam_SummitsPlusMinus150bp_results.csv", header=T, stringsAsFactors = F) %>%
                      dplyr::mutate(Genotype = 'H3.3', Fraction = 'Pellet')

head(H3.3_sup_smallFrag_summit_data)

```

```{r Merge Sup Data}

merged_Data <- rbind(H3.2_sup_smallFrag_summit_data, H3.3_sup_smallFrag_summit_data)
...
merged_Data %>%
  ...
  labs(title = 'H3.2 vs H3.3 Significant Summits: Small Fragment Sup Fraction', y = 'Summits', x  = 'Genotype', fill='Enrichment') +
...
merged_Data %>%
  ...
  facet_grid(~Chr) +
  labs(title = 'H3.2 vs H3.3 Significant Summits per Chromosome: Small Fragment Sup Fraction', y = 'Summits',x  = 'Genotype', 
       fill = 'Regulation') +
...
merged_Data %>%
  ...
  geom_point(data = function(x){x %>% dplyr::filter(padj < 0.05) %>% dplyr::filter(log2FoldChange < -1)},
             color = "#ff0000", 
             size=1, alpha=0.7) +
  labs(title = 'H3.2 vs H3.3 Summits: Small Fragment Sup Fraction') +
  theme(plot.title = element_text(hjust = 0.5)) +
