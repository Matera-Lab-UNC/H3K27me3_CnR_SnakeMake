---
title: "poolGenotypes DESeq2 pellet allFragmentBam PHO Analysis"
...
sample.id <- read.csv("/nas/longleaf/home/bdmcmi/work/Benjamin/H3K27me3_CutnRun/Pel_Sup_IgGcon_Analysis_20221202/Bedtools_DESeq2_broadPeak_Results/pellet_samples_IgGcon_Bedtools_DESeq2_merged10kb_broadPeak_sampleSheet.csv", header=T, stringsAsFactors = F)
...
annotation <- read.table("221129_CONCAT_Short_frag_summit_intevals_overlapping_Pho_Fig5CandS10_MERGED_OVERLAPPING.bed") %>%
...
write.csv(as.data.frame(H3K36R.resOrdered), file = paste0("DESeq2_Results/",currentDate,"_K36R_HWT_pellet_IgGcon_DESeq2_allFragBam_PHO_results.csv")
                )
...
write.csv(as.data.frame(H33BK36R.resOrdered), file = paste0("DESeq2_Results/",currentDate,"_Adelta_H33BK36R_pellet_IgGcon_DESeq2_allFragBam_PHO_results.csv")
                )
...
## Added on 08-16-2022 simply to count number of up and down enriched peaks that correspond to MA-plots and bar charts!!!
...
H3K36R.resOrdered %>%
  ...
  write.csv(., file = paste0("DESeq2_Results/",currentDate,"_K36R_HWT_pellet_IgGcon_DESeq2_allFragBam_PHO_enrichment.csv")
                )
...
H33BK36R.resOrdered %>%
  ...
  dplyr::filter(log2FoldChange > 1 | log2FoldChange < -1) %>%
  dplyr::mutate(Enrichment = ifelse(log2FoldChange > 1, 'Up', 'Down')) %>%
  write.csv(., file = paste0("DESeq2_Results/",currentDate,"_Adelta_H33BK36R_pellet_IgGcon_DESeq2_allFragBam_PHO_enrichment.csv")
                )
...
H3K36R.resOrdered %>%
  ...
  EnhancedVolcano(
    ...
    x = 'log2FoldChange',
    y = 'pvalue',
    title = 'H3K36R versus HWT - Pellet')
...
H33BK36R.resOrdered %>%
  ...
  EnhancedVolcano(
    ...
    x = 'log2FoldChange',
    y = 'pvalue',
    title = 'H33BK36R versus Adelta - Pellet')
