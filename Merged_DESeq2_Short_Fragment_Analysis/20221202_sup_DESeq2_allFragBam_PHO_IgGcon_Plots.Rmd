---
title: "Plots: DESeq2 sup allFragmentBam PHO Analysis"
...
H3.2_sup_allFrag_PHO_data <- read.csv("DESeq2_Results/20221205_K36R_HWT_sup_IgGcon_DESeq2_allFragBam_PHO_results.csv", header=T, stringsAsFactors = F) %>%
                      dplyr::mutate(Genotype = 'H3.2', Fraction = 'Sup')

head(H3.2_sup_allFrag_PHO_data)

```

```{r read in H3.3 DESeq2 Sup Fraction Data}

H3.3_sup_allFrag_PHO_data <- read.csv("DESeq2_Results/20221205_Adelta_H33BK36R_sup_IgGcon_DESeq2_allFragBam_PHO_results.csv", header=T, stringsAsFactors = F) %>%
                      dplyr::mutate(Genotype = 'H3.3', Fraction = 'Sup')

head(H3.3_sup_allFrag_PHO_data)

```

```{r Merge Pellet Data}

merged_Data <- rbind(H3.2_sup_allFrag_PHO_data, H3.3_sup_allFrag_PHO_data)
...
merged_Data %>%
  ...
  labs(title = 'H3.2 vs H3.3 Significant PHO Sites: All Fragment Sup Fraction', y = 'PHO Sites', x  = 'Genotype', fill='Enrichment') +
...
merged_Data %>%
  ...
  facet_grid(~Chr) +
  labs(title = 'H3.2 vs H3.3 Significant PHO Sites per Chromosome: All Fragment Sup Fraction', y = 'PHO Sites',x  = 'Genotype', 
       fill = 'Regulation') +
...
merged_Data %>%
  ...
  geom_point(data = function(x){x %>% dplyr::filter(padj < 0.05) %>% dplyr::filter(log2FoldChange < -1)},
             color = "#ff0000", 
             size=1, alpha=0.7) +
  labs(title = 'H3.2 vs H3.3 PHO Sites: All Fragment Sup Fraction') +
  theme(plot.title = element_text(hjust = 0.5)) +
