---
title: "poolGenotypes DESeq2 pellet smallFragmentBam PHO Analysis"
...
sample_id <- read.csv("pellet_samples_IgGcon_DESeq2_smallFrag_sampleSheet.csv", header=T, stringsAsFactors = F)
...
write.csv(as.data.frame(H3K36R.resOrdered), file = paste0("DESeq2_Results/",currentDate,"_K36R_HWT_pellet_IgGcon_DESeq2_smallFragBam_PHO_results.csv")
                )
...
write.csv(as.data.frame(H33BK36R.resOrdered), file = paste0("DESeq2_Results/",currentDate,"_Adelta_H33BK36R_pellet_IgGcon_DESeq2_smallFragBam_PHO_results.csv")
                )
...
H3K36R.resOrdered %>%
  ...
  write.csv(., file = paste0("DESeq2_Results/",currentDate,"_K36R_HWT_pellet_IgGcon_DESeq2_smallFragBam_PHO_enrichment.csv")
                )
...
H33BK36R.resOrdered %>%
  ...
  dplyr::filter(log2FoldChange > 1 | log2FoldChange < -1) %>%
  dplyr::mutate(Enrichment = ifelse(log2FoldChange > 1, 'Up', 'Down')) %>%
  write.csv(., file = paste0("DESeq2_Results/",currentDate,"_Adelta_H33BK36R_pellet_IgGcon_DESeq2_smallFragBam_PHO_enrichment.csv")
                )
